...
interface FacebookPostFieldsResponse {
  ...
  id?: string; // The post ID is also returned
  error?: MetaApiErrorDetail;
}
...
interface HistoricalPostRow {
  ...
  views: number | null; // Assuming these are numeric or null in DB
  reach: number | null;
...
async function fetchNewPostsFromMetaAPI(
  since?: string // This 'since' comes from lastFetchTimestamp
): Promise<PostDataForScoring[]> {
  console.log(`Fetching new posts from Meta API${since ? ` since ${since}` : ' (first run or no previous timestamp)'}...`);
  ...
  if (since) {
    try {
      ...
      console.warn(`Invalid 'since' timestamp. Defaulting to ${defaultSinceDate.toISOString()} (Unix: ${sinceUnixTimestamp})`);
    }
  } else {
    const defaultSinceDate = new Date('2024-01-01T00:00:00Z');
    sinceUnixTimestamp = Math.floor(defaultSinceDate.getTime() / 1000);
    console.log(`No 'since' timestamp provided. Defaulting to ${defaultSinceDate.toISOString()} (Unix: ${sinceUnixTimestamp})`);
  }
...
async function getInstagramBusinessAccountId(
  ...
  try {
    ...
    if (!response.ok) {
      console.error("Error getting Instagram Business Account ID:", data.error || `Status: ${response.status}`);
      return null;
    }
    if (data.instagram_business_account?.id) {
      return data.instagram_business_account.id;
    }
    console.log("Instagram Business Account ID not found in response for page:", pageId);
    return null;
  } catch (error) {
...
async function fetchFacebookPosts(
  ...
  const initialUrl = `https://graph.facebook.com/v22.0/${pageId}/published_posts?fields=id,created_time,permalink_url,message,is_published,is_expired&limit=25${since ? `&since=${since}` : ''}&access_token=${accessToken}`;
  ...
  while (nextUrl) {
    fetchedPageCount++;
    console.log(`[fetchFacebookPosts] Fetching page ${fetchedPageCount}... URL: ${nextUrl.substring(0,150)}...`);
    try {
      const response = await fetch(nextUrl);
      console.log(`[fetchFacebookPosts] Response status for page ${fetchedPageCount}: ${response.status} ${response.statusText}`);
      if (!response.ok) {
        let errorData;
        try {
      ...
      if (data.data && Array.isArray(data.data)) {
        console.log(`[fetchFacebookPosts] Found ${data.data.length} posts on page ${fetchedPageCount}.`);
        ...
        console.log(`[fetchFacebookPosts] No 'data' array in the response or it's empty for page ${fetchedPageCount}.`);
      }
      ...
      console.error(`[fetchFacebookPosts] Exception during fetch for page ${fetchedPageCount} from ${nextUrl || initialUrl}:`, error);
      break;
    }
  }
  return posts;
...
async function fetchFacebookPostInsights(
  ...
  const insightsMetrics = 'post_impressions,post_impressions_unique,post_reactions_by_type_total,post_clicks_by_type';
  ...
  try {
    ...
    const postFieldsJson: FacebookPostFieldsResponse = await postFieldsResponse.json();

    if (!insightsResponse.ok) {
        console.error(`Error fetching insights for post ${postId}:`, insightsJson.error || `Status: ${insightsResponse.status}`);
    }
    if (!postFieldsResponse.ok) {
        console.error(`Error fetching post fields (comments/shares) for post ${postId}:`, postFieldsJson.error || `Status: ${postFieldsResponse.status}`);
    }
    ...
    if (insightsJson.data) {
      const impressionsData = insightsJson.data.find(item => item.name === 'post_impressions');
      if (typeof impressionsData?.values?.[0]?.value === 'number') {
        views = impressionsData.values[0].value;
      }
      const reachData = insightsJson.data.find(item => item.name === 'post_impressions_unique');
      if (typeof reachData?.values?.[0]?.value === 'number') {
        reach = reachData.values[0].value;
      }
      const reactionsData = insightsJson.data.find(item => item.name === 'post_reactions_by_type_total');
      if (reactionsData?.values?.[0]?.value && typeof reactionsData.values[0].value === 'object') {
        totalReactions = Object.values(reactionsData.values[0].value).reduce((sum, count) => sum + (Number(count) || 0), 0);
      }
      const clicksData = insightsJson.data.find(item => item.name === 'post_clicks_by_type');
      if (clicksData?.values?.[0]?.value && typeof clicksData.values[0].value === 'object') {
        const clicksValue = clicksData.values[0].value as Record<string, number>;
        linkClicks = (clicksValue.link_clicks || 0) + (clicksValue['other clicks'] || 0) ;
      }
    }
    ...
    console.error(`Exception fetching insights bundle for Facebook post ${postId}:`, error);
...
async function fetchInstagramPosts(
  ...
  const posts: PostDataForScoring[] = [];
  const initialUrl = `https://graph.facebook.com/v22.0/${igBusinessId}/media?fields=id,timestamp,permalink,media_type,caption,media_product_type&limit=25${since ? `&since=${since}` : ''}&access_token=${accessToken}`;
  let nextUrl: string | undefined = initialUrl;
  ...
  while (nextUrl) {
    fetchedPageCountIG++;
    console.log(`[fetchInstagramPosts] Fetching IG page ${fetchedPageCountIG}... URL: ${nextUrl.substring(0,150)}...`);
    try {
      const response = await fetch(nextUrl);
      console.log(`[fetchInstagramPosts] Response status for IG page ${fetchedPageCountIG}: ${response.status} ${response.statusText}`);
      if (!response.ok) {
        let errorData;
        try {
          errorData = await response.json() as MetaApiErrorResponse;
      ...
      const data: MetaFeedResponse = await response.json();
      if (data.data && Array.isArray(data.data)) {
        console.log(`[fetchInstagramPosts] Found ${data.data.length} IG media on page ${fetchedPageCountIG}.`);
        for (const media of data.data) {
          if (media.media_product_type === 'STORY' || media.media_product_type === 'REELS_AD' || media.media_product_type === 'AD') {
            continue;
          }
          try {
        ...
        console.log(`[fetchInstagramPosts] No 'data' array in the IG response or it's empty for page ${fetchedPageCountIG}.`);
      }
      nextUrl = data.paging?.next || undefined;
      if (!nextUrl) {
      ...
      console.error(`[fetchInstagramPosts] Exception during fetch for IG page ${fetchedPageCountIG} from ${nextUrl || initialUrl}:`, error);
      break;
    }
  }
  return posts;
  ...
  try {
    ...
    if (data.data && Array.isArray(data.data)) {
      for (const metric of data.data) {
        ...
        switch (metric.name) {
          case 'views': newViews = Number(value) || 0; break;
          case 'reach': reach = Number(value) || 0; break;
          case 'likes': likes = Number(value) || 0; break;
          case 'comments': comments = Number(value) || 0; break;
          case 'saved': saved = Number(value) || 0; break;
          case 'total_interactions': totalInteractions = Number(value) || 0; break;
        }
      }
    }
    const calculatedEngagementSum = likes + comments + saved;
    const finalEngagement = totalInteractions > 0 ? totalInteractions : calculatedEngagementSum;
...
async function fetchAllHistoricalPosts(supabaseClient: SupabaseClient): Promise<PostDataForScoring[]> {
    const allHistoricalPostsMapped: PostDataForScoring[] = [];
    const CHUNK_SIZE = 1000; // Align with PostgREST's typical default limit
    let offset = 0;
    ...
    while (keepFetching) {
        // Note: supabaseClient is passed, ensure it's the correct Supabase client instance
        const { data: chunk, error, count } = await supabaseClient
            .from("social_posts")
            .select("post_id, platform, publish_time, permalink, views, reach, interactions, link_clicks", { count: 'exact' })
            .range(offset, offset + CHUNK_SIZE - 1);

        if (error) {
            console.error(`Error fetching chunk of historical data (offset: ${offset}):`, error.message);
            throw new Error(`Failed to fetch chunk of historical data: ${error.message}`);
        }

        if (totalCountFromDB === null && count !== null) {
            totalCountFromDB = count; 
            console.log(`Total historical posts in DB as per count: ${totalCountFromDB}`);
        }

        if (chunk && chunk.length > 0) {
            const mappedChunk: PostDataForScoring[] = chunk.map((p: HistoricalPostRow) => ({
                post_id: p.post_id,
                platform: p.platform,
            ...
            console.log(`Fetched DB chunk: ${mappedChunk.length} rows, current offset: ${offset}. Total mapped so far: ${allHistoricalPostsMapped.length}.`);
            
            offset += CHUNK_SIZE; 

            if (chunk.length < CHUNK_SIZE || (totalCountFromDB !== null && allHistoricalPostsMapped.length >= totalCountFromDB)) {
                keepFetching = false;
            }
        } else {
            keepFetching = false;
    ...
    console.log(`Finished fetching all historical posts from DB. Total mapped: ${allHistoricalPostsMapped.length}`);
    return allHistoricalPostsMapped;
}


export async function POST() {
  const supabase = createClient(); // This is your server-side client
  ...
  if (authError || !user || !user.email) {
    ...
    return NextResponse.json({ error: "Unauthorized", details: "User session not found or email missing." }, { status: 401 });
  }
  const allowed = await isUserAllowed(user.email, supabase);
  if (!allowed) {
    console.error(`Analysis run: Forbidden access attempt by ${user.email}.`);
    return NextResponse.json({ error: "Forbidden", details: "User not authorized for this action." }, { status: 403 });
  }
  ...
  try {
    ...
    console.log("Analysis run: Last fetch timestamp was:", lastFetchTimestamp ?? "Never (will use default since date)");

    // MODIFIED SECTION: Use the new paginated fetcher
    console.log("Analysis run: Fetching historical data for scoring context using pagination...");
    const historicalPosts: PostDataForScoring[] = await fetchAllHistoricalPosts(supabase); // Pass the supabase client
    console.log(`Analysis run: Fetched ${historicalPosts.length} total historical posts for scoring context (after pagination).`);
    ...
    if (!newPosts || newPosts.length === 0) {
      ...
      const { error: updateTimestampErrorNoNew } = await supabase
        .from('app_metadata')
        .upsert({ key: LAST_FETCH_TIMESTAMP_KEY, value_timestamp: currentTimestampISO, updated_at: currentTimestampISO }, { onConflict: 'key' });
      if (updateTimestampErrorNoNew) {
        console.error("Analysis run: Failed to update last fetch timestamp (no new posts):", updateTimestampErrorNoNew.message);
      } else {
        console.log("Analysis run: Successfully updated last fetch timestamp to (no new posts):", currentTimestampISO);
      }
      return NextResponse.json({ message: "No new posts found to process. Last fetch time updated." }, { status: 200 });
    }

    console.log(`Analysis run: Processing and scoring ${newPosts.length} new posts...`);
    const scoredRankedPosts: ScoredPost[] = processAndScoreBatch(
      newPosts,
      historicalPosts,
      ENGAGEMENT_WEIGHTS
    );
    console.log(`Analysis run: Scored and ranked ${scoredRankedPosts.length} new posts.`);
    ...
    if (finalUpsertData.length > 0) {
      ...
      console.log("Analysis run: No new data was processed and scored for upsert (this might be unexpected if new posts were fetched).");
    }

    console.log("Analysis run: Updating last fetch timestamp (after processing new posts)...");
